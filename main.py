...
@app.get("/")
def health_check():
    """Simple endpoint so Render’s health checks pass."""
    key_ok = bool(openai.api_key)
    return {
        "status": "ok",
        "service": "CrediLens Scoring API is running",
...
def calc_income_metrics(transactions):
    ...
    monthly_income = [sum(t.amount for t in txs if t.type=="credit") for txs in months.values()]
    ...
    return {"avg_income": round(float(avg_income),2), "income_volatility": round(vol,3),
            "income_diversity": diversity, "income_trend": round(trend,3)}
...
def calc_cashflow_metrics(transactions):
    ...
    return {"savings_rate": round(savings_rate,3), "liquidity_buffer": round(liquidity,2),
            "negative_balance_freq": round(neg_freq,3)}
...
def calc_spending_metrics(transactions):
    ...
    return {"essential_spending_ratio": round(essential_ratio,3), "subscription_burden": round(subscription_burden,3),
            "discretionary_control": round(discretionary_control,3)}
...
def calc_responsibility_metrics(transactions):
    ...
    positive_saving_months = sum(1 for m,txs in group_by_month(transactions).items()
                                 if sum(t.amount for t in txs if t.type=="credit") > sum(t.amount for t in txs if t.type=="debit"))
    saving_consistency = positive_saving_months / max(len(group_by_month(transactions)),1)
    bill_timely = 0.9
    return {"overdraft_avoidance": round(overdraft_avoidance,3),
            "saving_consistency": round(saving_consistency,3),
            "bill_punctuality": round(bill_timely,3)}
...
def generate_reason_codes(metrics: Dict[str,Any], pd: float, credit_score_external: float | None, fraud_flags: List[str]):
    prompt_lines = [
        ...
        "Input: JSON with behavioral submetrics, PD (0-1), optional external credit score, and fraud flags.",
        "Output: JSON with keys: primary_reasons (list), improvement_suggestions (list), confidence (0-1).",
        "Be concise; avoid raw metric numbers."
    ]
    input_payload = {
        "metrics": metrics, "PD": pd,
        "external_score": credit_score_external, "fraud_flags": fraud_flags
    }
    prompt = "\n".join(prompt_lines) + "\n\nINPUT:\n" + str(input_payload) + "\n\nReturn JSON only."
    ...
    try:
        ...
        if m:
            ...
            return {"primary_reasons":["Insufficient data"],"improvement_suggestions":[],"confidence":0.6}
    except Exception:
...
async def score_endpoint(req: ScoreRequest):
    ...
    behavioral_index = (income_stab*0.28 + cash_health*0.28 + spending_beh*0.22 + responsibility*0.22)
    fraud_adj = max(0, 1 - (req.fraud_score / 100) * 0.4)
    ...
    reason_payload = generate_reason_codes(
        metrics={"income": income, "cash": cash, "spend": spend, "responsibility": resp,
